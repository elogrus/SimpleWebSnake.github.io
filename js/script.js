...
let snakePiecesList = [{x: 250, y: 250},{x: 250, y: 275}];
let applesList = [];
...
function removeApple(needToRemoveApple) {
    let removed = applesList.splice(applesList.findIndex((apple) => apple.x == needToRemoveApple.x && apple.y == needToRemoveApple.y, 1))
...
function needCutSnake() {
    for (piece of snakePiecesList) {
        if (piece != snakePiecesList[0] && piece.x == snakePiecesList[0].x && piece.y == snakePiecesList[0].y) {
            return piece;
        }
    }
    return false;
...
function isWallCollusion() {
    if (snakePiecesList[0].x < 0 || snakePiecesList[0].y < 0 || snakePiecesList[0].x > 475 || snakePiecesList[0].y > 475) return true;
    return false;
}
...
function drawGameFrame() {
    ...
    // Отрисовка змеи
    if (!eatenApple) {
        ctx.clearRect(snakePiecesList.at(-1).x,snakePiecesList.at(-1).y,25,25);     // Убираю последний кусок змеи
        snakePiecesList.pop()                                                      
    }

    switch (direction) {                                                       // Рисую голову змеи взависимости от направления движения
        case 'top':
            snakePiecesList.unshift({x: snakePiecesList[0].x, y: snakePiecesList[0].y - 25})
            break;
        case 'bottom':
            snakePiecesList.unshift({x: snakePiecesList[0].x, y: snakePiecesList[0].y + 25})
            break;
        case 'right':
            snakePiecesList.unshift({x: snakePiecesList[0].x + 25, y: snakePiecesList[0].y})
            break;
        case 'left':
            snakePiecesList.unshift({x: snakePiecesList[0].x - 25, y: snakePiecesList[0].y})
            break;
